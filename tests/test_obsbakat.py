def test_step1(client):
    response = client.get("/api/v1/flow/obsbakat/step1")
...
def test_step2(client):
    response = client.get("/api/v1/flow/obsbakat/step2")
    assert response.status_code == 200
    assert "next_prompt_hint" in response.json()

def test_step3(client):
    response = client.get("/api/v1/flow/obsbakat/step3")
    assert response.status_code == 200
    assert "next_prompt_hint" in response.json()

def test_step4(client):
    response = client.get("/api/v1/flow/obsbakat/step4")
    assert response.status_code == 200
