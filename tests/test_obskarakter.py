...
def test_step1(client):
    response = client.get("/result/v1/flow/obskarakter/step1_wawancara")
...
def test_step2_valid(client):
    ...
    response = client.get(f"/result/v1/flow/obskarakter/step2?scores={json.dumps(scores)}")
    assert response.status_code == 200
    assert "chart_url" in response.json()

def test_step2_invalid_json(client):
    response = client.get("/result/v1/flow/obskarakter/step2?scores=invalid")
